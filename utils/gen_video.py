...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Tool to generate qualitative results videos"
    )
    parser.add_argument(
        "--pred", help=".pkl file to load predictions from", required=True
    )
    parser.add_argument("--cfg", default="config.yaml", help="Config file")
    parser.add_argument(
        "--cover", default="tusimple_cover.png", help="Cover image file"
    )
    parser.add_argument("--out", default="video.avi", help="Output filename")
    parser.add_argument(
        "--view", action="store_true", help="Show predictions instead of creating video"
    )
    parser.add_argument(
        "--length", type=int, help="Length of the output video (seconds)"
    )
...
def main():
    ...
    if not args.view:
        video = create_video(
            args.out,
            width,
            height + (legend.shape[0] if legend is not None else 0),
            args.fps,
        )
    ...
    if args.length is not None and args.clips is not None:
        ...
        selected_clip_ids = np.random.choice(
            all_clip_ids, size=args.clips, replace=False
        )
        frame_idxs = (
            np.repeat(selected_clip_ids, clip_length).reshape(args.clips, clip_length)
