...
def get_anchors_use_frequency(cfg, split="train", t_pos=15.0, t_neg=20.0):
    model = cfg.get_model()
...
def save_mask(cfg_path, output_path):
    cfg = Config(cfg_path)
    frequency = get_anchors_use_frequency(cfg, split="train", t_pos=30.0, t_neg=35.0)
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Compute anchor frequency for later use as anchor mask"
    )
    parser.add_argument(
        "--output", help="Output path (e.g., `anchors_mask.pt`", required=True
    )
