...
MAX_CHECKPOINT_INTERVAL = 30 * 60  # half hour
...
class Experiment:
    def __init__(
        ...
        self.tensorboard_path = os.path.join(tensorboard_dir, exp_name)
    ...
    def setup_exp_dir(self):
        if os.path.exists(self.exp_dirpath):
            if self.mode == "train":
                print("Experiment `{}` already exists.".format(self.name))
                exit()
        else:
    ...
    def epoch_end_callback(self, epoch, max_epochs, model, optimizer, scheduler):
        ...
        if self.last_checkpoint_time is None:
            time_since_last_checkpoint = float("inf")
        else:
            time_since_last_checkpoint = time.time() - self.last_checkpoint_time

        if time_since_last_checkpoint >= MAX_CHECKPOINT_INTERVAL or epoch == max_epochs:
    ...
    def save_epoch_results(self, dataset, predictions, epoch):
        ...
        epoch_results_path = os.path.join(
            self.results_dirpath, "epoch_{:04d}".format(epoch)
        )
        predictions_dir = os.path.join(
            epoch_results_path, "{}_predictions".format(dataset.split)
        )
        ...
        for key in metrics:
            if isinstance(metrics[key], float):
                self.tensorboard_writer.add_scalar(
                    "{}_metrics/{}".format(dataset.split, key), metrics[key], epoch
                )
        # save metrics
        metrics_path = os.path.join(
            epoch_results_path, "{}_metrics.json".format(dataset.split)
        )
