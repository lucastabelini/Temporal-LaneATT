...
def match_proposals_with_targets(model, proposals, targets, t_pos=15.0, t_neg=20.0):
    ...
    targets_starts = targets[:, 3] * model.n_strips
    # proposals_starts = proposals[:, 3] * model.n_strips
    # starts = torch.max(targets_starts, proposals_starts).round().long()
    starts = targets_starts.round().long()
    ends = (targets[:, 4] * model.n_strips).round().long()
    lengths = ends - starts + 1
    ends[lengths < 0] = starts[lengths < 0] - 1
    lengths[
        lengths < 0
    ] = 0  # a negative number here means no intersection, thus zero length
    ...
    all_indices = torch.arange(
        valid_offsets_mask.shape[0], dtype=torch.long, device=targets.device
    )
    ...
    #   if length is zero, the previous line would put a one where it shouldnt be.
    ...
    distances = torch.abs((targets - proposals) * valid_offsets_mask.float()).sum(
        dim=1
    ) / (
        lengths.float() + 1e-9
    )  # avoid division by zero
    distances[lengths == 0] = INFINITY
    invalid_offsets_mask = invalid_offsets_mask.view(
        num_proposals, num_targets, invalid_offsets_mask.shape[1]
    )
